...
from obstacles import Building, Fireball, EnergyCore, PowerUp
...
class ParallaxBackground:
    ...
    def __init__(self, width, height):
        ...
        # Far buildings layer (silhouettes)
        self.buildings_far = []
        x = 0
        while x < width + 100:
    ...
    def update(self, dt, speed_mult):
        ...
        # Scroll far background layer
        for bldg in self.buildings_far:
            bldg["x"] -= 80 * speed_mult * dt
            
        if self.buildings_far and self.buildings_far[0]["x"] + self.buildings_far[0]["w"] < 0:
            last_x = self.buildings_far[-1]["x"] + self.buildings_far[-1]["w"]
            w = random.randint(40, 90)
            h = random.randint(100, 260)
            self.buildings_far.pop(0)
            self.buildings_far.append({"x": last_x + random.randint(10, 30), "w": w, "h": h})

    def draw(self, surface, elapsed_time, is_ignited):
        # Determine active Environment Zone
        if elapsed_time < 35:
            # ZONE 1: Night City
        ...
        # 2. Draw Stars / Space Dust
        for star in self.stars:
            color = (255, 255, 200) if elapsed_time >= 70 else (180, 210, 255)
            pygame.draw.circle(surface, color, (int(star[0]), int(star[1])), int(star[2]))

        # 3. Draw Far City Silhouettes (Zone 1 & Zone 2)
        if elapsed_time < 70:
            for bldg in self.buildings_far:
                ...
                # Small windows
                if elapsed_time >= 35:
                    for wy in range(self.height - bldg["h"] + 10, self.height - 10, 25):
                        pygame.draw.rect(surface, (255, 0, 120), (bldg["x"] + 8, wy, 4, 8))

        # 4. Neon Synthwave Ground Grid for Zone 2
        if 35 <= elapsed_time < 70:
            grid_y = self.height - 40
            pygame.draw.line(surface, (255, 0, 150), (0, grid_y), (self.width, grid_y), 2)

        # 5. Earth Atmosphere Curve for Zone 3
        if elapsed_time >= 70:
            pygame.draw.arc(surface, (0, 180, 255), (-100, self.height - 80, self.width + 200, 200), 0, math.pi, 4)

        return zone_name


class BluePoint(pygame.sprite.Sprite):
    """Blue Point collectible worth +2 points."""
...
class Game:
    def __init__(self):
        ...
        self.fullbody_ironman_img = self.create_fullbody_ironman_surface(140, 310)
        
        self.state = 'START'
        self.running = True
        
        self.start_btn_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT - 75, 200, 50)
        self.reset_game()
    ...
    def create_fullbody_ironman_surface(self, w, h):
        ...
        pygame.draw.ellipse(surf, (0, 240, 255, 120), (cx - 25, h - 18, 50, 10))
        ...
        pygame.draw.polygon(surf, DARK_RED, [(cx - 35, h - 70), (cx - 15, h - 70), (cx - 12, h - 10), (cx - 40, h - 10)])
        pygame.draw.polygon(surf, DARK_RED, [(cx + 15, h - 70), (cx + 35, h - 70), (cx + 40, h - 10), (cx + 12, h - 10)])
        pygame.draw.polygon(surf, ARMOR_RED, [(cx - 32, h - 65), (cx - 18, h - 65), (cx - 15, h - 12), (cx - 36, h - 12)])
        pygame.draw.polygon(surf, ARMOR_RED, [(cx + 18, h - 65), (cx + 32, h - 65), (cx + 36, h - 12), (cx + 15, h - 12)])

        pygame.draw.polygon(surf, GOLD_BRIGHT, [(cx - 32, h - 75), (cx - 16, h - 75), (cx - 18, h - 60), (cx - 30, h - 60)])
        pygame.draw.polygon(surf, GOLD_BRIGHT, [(cx + 16, h - 75), (cx + 32, h - 75), (cx + 30, h - 60), (cx + 18, h - 60)])

        pygame.draw.polygon(surf, GOLD_DARK, [(cx - 30, h - 140), (cx - 12, h - 140), (cx - 14, h - 75), (cx - 32, h - 75)])
        pygame.draw.polygon(surf, GOLD_DARK, [(cx + 12, h - 140), (cx + 30, h - 140), (cx + 32, h - 75), (cx + 14, h - 75)])

        pygame.draw.polygon(surf, DARK_RED, [(cx - 38, h - 230), (cx + 38, h - 230), (cx + 25, h - 135), (cx - 25, h - 135)])
        pygame.draw.polygon(surf, ARMOR_RED, [(cx - 34, h - 225), (cx + 34, h - 225), (cx + 22, h - 140), (cx - 22, h - 140)])
        
        for y_off in [155, 172, 189]:
            pygame.draw.polygon(surf, GOLD_BRIGHT, [(cx - 18, h - y_off), (cx + 18, h - y_off), (cx + 15, h - y_off + 10), (cx - 15, h - y_off + 10)])

        pygame.draw.ellipse(surf, ARMOR_RED, (cx - 58, h - 240, 32, 22))
        pygame.draw.ellipse(surf, ARMOR_RED, (cx + 26, h - 240, 32, 22))
        pygame.draw.ellipse(surf, SILVER_TRIM, (cx - 54, h - 236, 24, 14), width=2)
        pygame.draw.ellipse(surf, SILVER_TRIM, (cx + 30, h - 236, 24, 14), width=2)

        pygame.draw.polygon(surf, ARMOR_RED, [(cx - 52, h - 220), (cx - 36, h - 220), (cx - 42, h - 160), (cx - 56, h - 160)])
        pygame.draw.polygon(surf, ARMOR_RED, [(cx + 36, h - 220), (cx + 52, h - 220), (cx + 56, h - 160), (cx + 42, h - 160)])
        pygame.draw.polygon(surf, GOLD_BRIGHT, [(cx - 55, h - 160), (cx - 41, h - 160), (cx - 46, h - 120), (cx - 58, h - 120)])
        pygame.draw.polygon(surf, GOLD_BRIGHT, [(cx + 41, h - 160), (cx + 55, h - 160), (cx + 58, h - 120), (cx + 46, h - 120)])

        pygame.draw.polygon(surf, ARC_BLUE, [(cx, h - 200), (cx - 14, h - 216), (cx + 14, h - 216)])
        pygame.draw.polygon(surf, WHITE, [(cx, h - 202), (cx - 9, h - 214), (cx + 9, h - 214)])
        pygame.draw.polygon(surf, SILVER_TRIM, [(cx, h - 197), (cx - 17, h - 219), (cx + 17, h - 219)], width=2)

        pygame.draw.ellipse(surf, DARK_RED, (cx - 22, h - 280, 44, 48))
        pygame.draw.polygon(surf, GOLD_BRIGHT, [
            (cx - 16, h - 272), (cx + 16, h - 272),
            (cx + 18, h - 252), (cx + 10, h - 238),
            (cx - 10, h - 238), (cx - 18, h - 252)
        ])
    ...
    def spawn_entities(self, dt):
        ...
        if self.spawn_timer >= 0.7:
            ...
            if rand_val < 0.60:
                blue_pt = BluePoint(WIDTH + 30, random.randint(40, HEIGHT - 180), self.speed_multiplier)
    ...
    def draw_start_page(self):
        ...
        pygame.draw.rect(rules_box, (0, 200, 255), (0, 0, box_w, box_h), 2, border_radius=10)
        ...
        txt_head = font_head.render("MISSION RULES & CONTROLS", True, (0, 240, 255))
        self.screen.blit(txt_head, (WIDTH // 2 - txt_head.get_width() // 2, 82))

        self.screen.blit(self.fullbody_ironman_img, (WIDTH - 180, HEIGHT - 380))

        rules = [
            ("CONTROLS:", (255, 215, 0)),
            ("  - WASD / Arrow Keys or Hand Tracking to move.", (220, 220, 220)),
            ("  - Press SHIFT to activate Ignite Mode.", (220, 220, 220)),
            ("  - Press F11 to toggle Full Screen.", (220, 220, 220)),
            ("STAGES & OBJECTIVE:", (255, 215, 0)),
            ("  - Survive 100s across 3 unique Environments!", (220, 220, 220)),
            ("  - Zone 1: Night City | Zone 2: Cyber Grid | Zone 3: Atmosphere", (220, 220, 220)),
            ("LIVES SYSTEM:", (255, 60, 80)),
            ("  - You have 3 Lives! Obstacles drain 1 life + grant 1.5s i-frames.", (220, 220, 220)),
            ("SCORING SYSTEM:", (0, 255, 150)),
            ("  - Blue Orbs: +2 pts  |  Energy Cores: +50 pts", (220, 220, 220)),
            ("  - Power-Ups: +200 pts (Triggers 6s Ignite)", (220, 220, 220)),
            ("IGNITE MODE:", (255, 140, 0)),
            ("  - Invincible to obstacles + DOUBLE POINTS (2x)!", (220, 220, 220)),
        ]
        ...
        btn_color = (255, 70, 0) if self.start_btn_rect.collidepoint(mouse_pos) else (200, 40, 40)
        
        pygame.draw.rect(self.screen, btn_color, self.start_btn_rect, border_radius=12)
        pygame.draw.rect(self.screen, (255, 215, 0), self.start_btn_rect, 3, border_radius=12)
        
        font_btn = pygame.font.SysFont("Consolas", 22, bold=True)
        txt_btn = font_btn.render("START GAME", True, (255, 255, 255))
        self.screen.blit(txt_btn, (self.start_btn_rect.centerx - txt_btn.get_width() // 2, self.start_btn_rect.centery - txt_btn.get_height() // 2))
    ...
    def draw_game_over(self, is_victory=False):
        ...
        img_rect = self.gameover_img.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 10))
        ...
        txt_score = font_small.render(f"FINAL SCORE: {int(self.score)}", True, (255, 255, 255))
        txt_restart = font_small.render("Press SPACE to Play Again or ESC to Exit", True, (180, 180, 180))

        self.screen.blit(txt_title, (WIDTH // 2 - txt_title.get_width() // 2, HEIGHT // 2 - 140))
        self.screen.blit(txt_time, (WIDTH // 2 - txt_time.get_width() // 2, HEIGHT // 2 + 100))
        self.screen.blit(txt_score, (WIDTH // 2 - txt_score.get_width() // 2, HEIGHT // 2 + 130))
        self.screen.blit(txt_restart, (WIDTH // 2 - txt_restart.get_width() // 2, HEIGHT // 2 + 170))
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.state == 'START' and self.start_btn_rect.collidepoint(event.pos):
                        self.reset_game()
                        self.state = 'PLAYING'
                    ...
                    elif event.key == pygame.K_SPACE and self.state in ['GAMEOVER', 'VICTORY']:
                        self.reset_game()
                        self.state = 'PLAYING'

            self.gesture_ctrl.update()
            target_y = self.gesture_ctrl.target_y if self.gesture_ctrl.hand_detected else pygame.mouse.get_pos()[1]
            ...
            if self.state == 'START':
                ...
                self.speed_multiplier = 1.0 + (int(self.elapsed_time // 5) * 0.10)
                ...
                zone_title = self.background_mgr.draw(self.screen, self.elapsed_time, self.player.is_ignited)

                self.spawn_entities(dt)
                self.player.update(dt, target_y, hand_active=self.gesture_ctrl.hand_detected)
                self.obstacles.update(dt)
                self.collectibles.update(dt)
                
                self.handle_collisions()

                if self.invincible_timer <= 0 or int(self.invincible_timer * 12) % 2 == 0:
                ...
                time_txt = font_hud.render(f"TIME: {current_sec}s / 100s", True, (0, 220, 255))
                score_txt = font_hud.render(f"SCORE: {int(self.score)}", True, (255, 255, 255))
                speed_txt = font_hud.render(f"SPEED: {self.speed_multiplier:.2f}x", True, (255, 215, 0))
                ...
                if self.player.is_ignited:
                    ...
                    ignite_txt = ignite_font.render(f"IGNITE MODE: {self.player.ignite_timer:.1f}s", True, (255, 140, 0))
